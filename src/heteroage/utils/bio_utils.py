...
def construct_biosparse_topology(hallmark_dict, master_cpg_list):
    ...
    deterministic binary mask tensor and determines the structural complexity (Tier) for each branch.
    ...
    for name in sorted(hallmark_dict.keys()):
        ...
        # C. Sparse Mask Generation (Deterministic Fan-out Connection)
        branch_mask = torch.zeros(num_cpgs * 3, hidden_dim)
        fan_out = 3  # Each CpG connects only 3 neurons to prevent parameter explosion.
        
        if count > 0:
            cpg_indices = [cpg_to_idx[c] for c in valid_cpgs]
            
            for mod_offset in [0, num_cpgs, 2 * num_cpgs]:
                for i, cpg_idx in enumerate(cpg_indices):
                    row = cpg_idx + mod_offset
                    
                    # Modulo mapping
                    for j in range(fan_out):
                        col = (i * fan_out + j) % hidden_dim
                        branch_mask[row, col] = 1.0
