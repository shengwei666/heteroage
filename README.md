...
**Traditional epigenetic clocks** predominantly rely on linear models using exclusively Beta values, often limited to specific tissues or functioning as opaque 'black boxes.' To address these limitations, we present **HeteroAge-HAB**, a novel **Pan-Tissue** deep learning framework. Trained on a diverse atlas of **19 tissue types**, it employs **our previously developed heterogeneity adjustment model** to derive methylation heterogeneity metrics (CHALM and CAMDA) directly from average methylation (Beta). Crucially, to bridge the gap between prediction and biological insight, HeteroAge-HAB is architecturally structured around the Hallmarks of Aging. By processing multi-modal inputs through specialized attention branches, our model captures universal, non-linear aging dynamics across different biological contexts, providing a transparent, hallmark-specific breakdown of the aging process.
